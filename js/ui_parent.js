...
(function () {
  ...
  function renderSettings(sc, p) {
    ...
    sc.appendChild(h('div.card.col', h('h3', 'このプロフィール'), h('div.field', h('label', '開始レベル（これより前のワールドは自由に遊べます）'), h('select', { id: 'startSel', on: { change: (e) => { p.startLevel = e.target.value; App.save(); App.toast('変更しました', 'good'); } } }, Content.worlds.map(w => h('option', { value: w.key, selected: w.key === p.startLevel }, w.emoji + ' ' + w.sub)))),
      h('div.field', h('label', 'ワールドをやり直す（まぐれで先に進んでしまったとき）'),
        h('div.row.wrap', Content.worlds.filter(w => Engine.worldTouched(p, w.key)).map(w => h('button.btn.sm.ghost', { on: { click: () => App.modal({ emoji: '↩️', title: w.emoji + ' ' + w.sub + ' をやり直す？', body: 'このワールドのステージクリア・レッスンの星・ボステストの合格を取り消し、このワールドの単語の記憶（ゾンビ）も消して、最初のレッスンからやり直します。XP・コイン・なかま・バッジはそのまま残ります。直前の1回だけ「元に戻す」で取り消せます。', buttons: [{ label: 'やり直す', cls: 'primary', onClick: () => { Engine.resetWorld(p, w.key); App.toast(w.sub + ' を最初からにしました', 'good'); App.refresh(); } }, { label: 'やめる', cls: 'ghost' }] }) } }, w.emoji + ' ' + w.sub))),
        p.undo && Content.world(p.undo.key) ? h('button.btn.sm.gold', { style: { marginTop: '6px' }, on: { click: () => { const k = Engine.undoReset(p); App.toast(Content.world(k).sub + ' の進み具合を元に戻しました', 'good'); App.refresh(); } } }, '↩️ 元に戻す（' + Content.world(p.undo.key).sub + ' のやり直しを取り消す）') : null,
        h('div.tiny.muted', '進み具合と、そのワールドの単語の記憶（復習に出るゾンビ）を消します。ボステストの合格も取り消すので、その次のワールドはロックに戻ります（そちらの記録は残るので、再合格すれば元通り）。開始レベルは変わりません（戻したワールドから始めたいときは上の「開始レベル」で）')),
      h('div.field', h('label', 'ワールドをクリア済みにする（すでに分かっている内容のとき／間違えてやり直したとき）'),
        h('div.row.wrap', Content.worlds.filter(w => !Engine.testPassed(p, w.key)).map(w => h('button.btn.sm.ghost', { on: { click: () => App.modal({ emoji: '✅', title: w.emoji + ' ' + w.sub + ' をクリア済みにする？', body: 'このワールドの全ステージ（ボス）とボステストを合格ずみにして、次のワールドを開きます。単語の記憶はそのままです。', buttons: [{ label: 'クリア済みにする', cls: 'primary', onClick: () => { Engine.clearWorld(p, w.key); App.toast(w.sub + ' をクリア済みにしました', 'good'); App.refresh(); } }, { label: 'やめる', cls: 'ghost' }] }) } }, w.emoji + ' ' + w.sub))))));
  }
  ...
  function renderHelp(sc) {
    ...
    sc.appendChild(h('div.card.col',
      ...
      h('h3', '🎮 遊び方のしくみ'),
      h('ul', { style: { paddingLeft: '1.2em', margin: 0, lineHeight: 1.7 } }, li('マップのステージ = 20単語くらいのテーマ。まず「📖 おぼえる」（カードを見て ⭕ を押すだけ、間違いなし）→ 4〜5レッスン + ボス。レッスンは ★★（初回正解 80%）でクリア、届かなければ同じレッスンをもう一度。ボスは 80% で撃破、負けたら練習（70%）→再挑戦（設定「マスターモード」）'), li('正解すると攻撃、連続正解でコンボ（ダメージ↑・5コンボでXP↑）。まちがえても失格にはならず、その問題があとで出直します'), li('覚えた単語は 1日・3日・7日・14日・30日後に「ゾンビ」として戻ってきます（忘却曲線に合わせた復習）。復習にはよく覚えている単語も数問まざり、間違えた単語はその日のうちにまた出ます'), li('毎日3つのクエスト。全部達成で宝箱＋たまご。たまごは 3〜5回のバトルで「なかま」に孵化（最初のたまごと金のたまごは3回）'), li('各級の最後は本物の英検の形式（語彙文法・会話・並べかえ・リスニング・読解）を模したボステスト。70点で合格→次の級が開放'), li('「たび」は来年の海外旅行のためのフレーズ集。現地で通じたら「つうじた！」で大きなごほうび')),
      h('h3', '📚 収録内容'),
